...
# # duiying directed coupling: 1+gamma; gamma
...
# duiying ketiao coupling: 1; gamma       phd
def directed_chain(T,gamma):
    At = np.zeros((T,T))
    for t in range(1,T):
        At[t-1,t] = 1-gamma        
...
# # duiying ketiao coupling(all-to-all): N*N  1        euro_air
def all_to_all_chain(T):
    At = np.ones((T,T))
    return At
...
#         print(joints[:,t],'111111')
#         print(marginals[t],'222222')
#         print(conditionals[:,t],'333333')
...
#     print(joints.shape)
    ...
    for link in range(len(joints)):
#         print(link)
        markes = ['-o', '-s', '-^', '-d']
        ax[0].plot(joints[link], markes[link]);
        ax[1].plot(conditionals[link], markes[link]);
        ax[0].set_xlabel('layer, $t$',fontdict=font)
        ax[1].set_xlabel('layer, $t$',fontdict=font)
        ax[0].set_ylabel('joint centrality, $W_{it}$',fontdict=font)
        ax[1].set_ylabel(' conditional centrality, $Z_{it}$',fontdict=font)
        ax[0].legend(['node '+str(i) for i in np.arange(1,1+np.shape(joints)[0])],prop=font1)
        ax[1].legend(['node '+str(i) for i in np.arange(1,1+np.shape(joints)[0])],prop=font1)
...
def visualize_toy1(graph):
    ...
    plt.tight_layout()
    ax.set_axis_off()
#     plt.xticks(x, x, rotation=60)  # 这里是调节横坐标的倾斜度，rotation是度数
